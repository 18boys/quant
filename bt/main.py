...
def main(code="sz399300", start_cash=1000000, stake=100, commission_fee=0):
    cerebro = bt.Cerebro()  # 创建主控制器
    cerebro.optstrategy(MyStrategy, maperiod=range(10, 130, 10))  # 导入策略参数寻优
    # cerebro.addstrategy(MyStrategy)  # 将交易策略加载到回测系统中
    # 利用 AKShare 获取股票的后复权数据，这里只获取前 6 列
    # stock_hfq_df = ak.stock_zh_a_hist(symbol=code, adjust="hfq", start_date='20000101', end_date='20220528').iloc[:, :6]
    stock_hfq_df = ak.stock_zh_index_daily(symbol=code).iloc[:, :6]
    print(stock_hfq_df.iloc[0:2])
    # stock_hfq_df.to_csv('./data/300.data')
    ...
    start_date = datetime(2005, 1, 4)  # 回测开始时间
    end_date = datetime(2016, 11, 25)  # 回测结束时间
    ...
    # cerebro.addsizer(MyOnly)  # 设置买入数量
    print("期初总资金: %.2f" % cerebro.broker.getvalue())
    cerebro.run(maxcpus=1)  # 用单核 CPU 做优化
    # cerebro.run()  # 用单核 CPU 做优化
    print("期末总资金: %.2f" % cerebro.broker.getvalue())


if __name__ == '__main__':
    main(code="sz399300", start_cash=1000000, stake=1, commission_fee=0.001)
